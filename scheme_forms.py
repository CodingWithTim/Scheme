...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # (define x 2)
        # Pair(x, Pair(2, nil))
        # expression.rest.first = 2
        # (define x (+ 1 2))
        # Pair(x, Pair(Pair(+, Pair(1, Pair(2, nil))), nil))
        # expressions.rest.first = Pair(Pair(+, Pair(1, Pair(2, nil))))
        ...
        # (define (f x y) (+ x y))
        #
        #  Pair(Pair(f, Pair(x, Pair(y, nil))), Pair(+, Pair(x, Pair(y))))
...
def do_quote_form(expressions, env):
    ...
    # do_quote_form(read_line("((+ x 2))")
    # Pair(Pair(+, Pair(x, Pair(2, nil))), nil)
    # Pair('+', Pair('x', Pair(2, nil)))
    # do_quote_form(read_line("(+ x 2)"))
    # do_quote_form(Pair(+, Pair(x, Pair(2, nil))))
    # '+ x 2'

    if scheme_listp(expressions.first):
        return Pair(do_quote_form(expressions.first), nil)
    # END PROBLEM 5
...
def do_and_form(expressions, env):
    ...
    if is_scheme_false(scheme_eval(expressions.first, env)):
        return False
    if is_scheme_false(expressions.first):
        return False
...
def make_let_frame(bindings, env):
    ...
    # ((a 1) (b 2) (c 3))
    # Pair(Pair(a, Pair(1, nil)), Pair(b, Pair(2, nil)))
    ...
    while bindings:
        ...
        if scheme_symbolp(bindings.first.rest.first):
            values = Pair(env.lookup(bindings.first.rest.first), values)
        else:
